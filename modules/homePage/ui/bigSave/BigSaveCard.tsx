...
const BigSaveCard = ({ card, onAdd }: { card: DealCardType, onAdd: () => void }) => {
    const { id, image, title, price, rate } = card;
    ...
    return (
        <div className="group flex flex-col gap-5 items-center justify-center mx-5">
            ...
            <div className='text-start'>
                ...
                <p className='text-lg font-semibold text-primary'>LKR {price.toLocaleString('en-US')}</p>
                <p className='flex text-xs gap-2'>
                    <FaStar size={14} className='text-yellow-400' />
                    {rate}
                </p>
                <p className='text-xs text-gray-400'>100K sold</p>
            </div>
            <button
                type="button"
                onClick={onAdd}
